...
Return<Status> Vibrator::on(uint32_t timeout_ms) {
    // Wave number 12 for vibrations slightly stronger than stock
    mWaveNumber << 12 << std::endl;
    mTimeoutAndAmplitude << timeout_ms << " " << currentAmplitude << std::endl;
...
Return<void> Vibrator::perform(Effect effect, EffectStrength strength, perform_cb _hidl_cb) {
    // TODO: Where to use strength? Suppress unused variable warning for now
    (void) strength;
    uint32_t id;
    switch (effect) {
        case Effect::CLICK:
